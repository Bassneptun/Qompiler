...
const __TOKENS: [&str; 46] = [
    "//", "*/", "/*", "(", ")", "{", "}", "[", "]", ":", ";", ",", "=", "&", "*", "..", "let", "var", "const", "struct", "qbit", "void", "#", "macro", "gate", "H", "PX", "PY", "PZ", "CNT", "CY", "ID", "TOF", "RX", "RY", "RZ", "S", "T", "SDG", "TDG", ".", "if", "for", "in", "return", "break"
];
...
fn rm_comments(input: &str) -> String {
...
fn tokenize(input: &str) -> Vec<Token> {
    ...
    while i < input.len() {
        ...
        if found {
            if let Some((token_index, token_value)) = longest_match_token {
                if !_str.is_empty() {
                    tokens.push(Token { token: 50, value: _str.clone() });
                    _str.clear();
                }
                tokens.push(Token {
                    token: token_index,
    ...
    if !_str.is_empty() {
        tokens.push(Token { token: 50, value: _str });
    }
...
fn filter50s(mut tokens: Vec<Token>) -> Vec<Token> {
    ...
    for tok in tokens.iter_mut() {
        match tok.token{
            5 => current_scope += 1,
            6 => {current_scope -= 1; lookup = lookup.iter().filter(|x| x.0 <= current_scope).map(|x| x.to_owned()).collect();},
            50 => {

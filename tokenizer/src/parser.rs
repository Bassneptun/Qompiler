...
fn create_type(token: Vec<Token>) -> Option<Box<ASTNode>> {
    if token.len() == 1 {
        match token[0].token {
...
fn parse_arguments(tokens: Vec<Token>) -> (Vec<ASTNode>, usize) {
...
fn parse_51(tokens: Vec<Token>) -> (ASTNode, usize) {
    match tokens[1].token{
        3 => (ASTNode::FunctionCall {
            name: tokens[0].value.clone(),
            args: parse_arguments(tokens[2..].to_vec()).0,
        }, parse_arguments(tokens[2..].to_vec()).1 + 1),
        7 => (ASTNode::ArrayAccess {
            name: tokens[0].value.clone(),
            index: Box::new(ASTNode::ArrayIndex(tokens[2].value.parse().expect("this should be a number..."))),
        }, 3),
        40 => (ASTNode::StructAccess {
            name: tokens[0].value.clone(),
            member: tokens[2].value.clone(),
        }, 3),
        _ => (ASTNode::VariableCall {
            name: tokens[0].value.clone(),
        }, 1),
    }
}

fn elems(tokens: Vec<Token>) -> Vec<ASTNode> {
...
fn parse_array_decl(tokens: Vec<Token>, q: usize) -> (ASTNode, usize) {
    let index = if tokens[3].token == 14 {6} else {5};
    if tokens.iter().position(|x| x.token == 5).is_some(){
        if tokens.iter().position(|x| x.token == 10).unwrap() < tokens.iter().position(|x| x.token == 5).unwrap() {
            (ASTNode::Array{
            ...
            (ASTNode::Array{
                ...
                size: Box::new(ASTNode::Num(tokens[index].value.parse().unwrap())),
                type_: create_type(tokens[3..(index-1)].to_vec().clone()).unwrap(),
                elements: elems(tokens[tokens.iter().position(|x| x.token == 5).unwrap()..].to_vec().clone()),
            }, tokens.iter().position(|x| x.token == 10).unwrap() + 1 + q)
        }
    } else {
            (ASTNode::Array{
...
fn parse_variable_decl(tokens: Vec<Token>, q: usize) -> (ASTNode, usize) {
    if tokens[4].token == 7 { 
        return parse_array_decl(tokens, q);
    }
    if tokens[2].token == 10{ // semicolon found right after variable declaration
        return (ASTNode::VariableDecl {
            name: tokens[1].value.clone(),
            value: None,
            type_: None,
            token: tokens[2].token as i32,
        }, 3 + q);
    } else if tokens[2].token == 9{ // type specifier found, is expected yet
        if tokens[4].token == 10 || (tokens[3].token == 14 && tokens[5].token == 10){ // semicolon found right after variable declaration, or the same but with a pointer type
            return (ASTNode::VariableDecl {
                name: tokens[1].value.clone(),
                value: None,
        ...
        } else if tokens[2].token == 9 { // equal sign expected here, but not checked for. Will be checked later
            let index = if tokens[5].token == 12 {6} else {5};
            match tokens[index].token {
                13 => return (ASTNode::VariableDecl {
                    name: tokens[1].value.clone(),
                ...
                14 => return (ASTNode::VariableDecl {
                    name: tokens[1].value.clone(),
                ...
                25..=39 => return (ASTNode::VariableDecl {
                    name: tokens[1].value.clone(),
                    ...
                    type_: create_type(tokens[3..index].to_vec().clone()),
                    token: tokens[0].token as i32,
                }, parse_arguments(tokens[index+2..].to_vec()).1 + q + [3..index].to_vec().len() - 1),
                51 => return (ASTNode::VariableDecl {
                    name: tokens[1].value.clone(),
                    value: Some(Box::new(parse_51(tokens[index..].to_vec()).0)),
                    type_: create_type(tokens[3..index].to_vec().clone()),
                    token: tokens[0].token as i32,
                }, parse_51(tokens[index..].to_vec()).1 + q + [3..index].to_vec().len() - 1),
                52 => return (ASTNode::VariableDecl {
                    name: tokens[1].value.clone(),
                    value: Some(Box::new(ASTNode::Num(tokens[index].value.parse().unwrap()))),
                    type_: create_type(tokens[3..index].to_vec().clone()),
                    token: tokens[0].token as i32,
                }, 7 + q),
    ...
    } else if tokens[2].token == 12{ // equal sign found, type deduction is expected
...
fn parse_rval(tokens: Vec<Token>) -> (ASTNode, usize) {
    match tokens[0].token {
        13 => (ASTNode::Reference {
            name: tokens[1].value.clone(),
        }, 2),
        14 => (ASTNode::Dereference {
            name: tokens[1].value.clone(),
        }, 2),
        25..=39 => (ASTNode::GateCall {
            name: tokens[0].value.clone(),
            args: parse_arguments(tokens[2..].to_vec()).0,
        }, parse_arguments(tokens[2..].to_vec()).1),
        51 => {
            if tokens.len() > 2 {
                (parse_51(tokens[2..].to_vec()).0, parse_51(tokens[2..].to_vec()).1)
            } else {
        ...
        52 => {
            (ASTNode::Num(tokens[0].value.parse().unwrap()), 1)
        }
        _ => panic!("Invalid token: {:?}, {:?}", tokens[0].token, tokens[0].value),
    }
}

fn find_args_and_signature(tokens: Vec<Token>) -> (Vec<String>, Vec<ASTNode>) {
        ...
        match tok.token {
            ...
            20 => {
                if is_type {
                    ...
                    panic!("expected name but got compiler-reserved keyword: {:?}", tok.value);
                }
            },
...
fn find_bounds(tokens: Vec<Token>) -> (usize, usize) {
    let mut i = 0; 
    let mut j = 0;
    let mut k = 0;
    for tok in &tokens{
...
fn extract(node: ASTNode) -> Box<ASTNode> {
...
fn parse_function_def(tokens: Vec<Token>, q: usize) -> (ASTNode, usize) {
    let args: Vec<String> = find_args_and_signature(tokens.clone()).0;
    let signature = (create_type(vec![tokens[0].clone()]).unwrap() ,find_args_and_signature(tokens.clone()).1);
...
fn fetch_args_and_names(tokens: Vec<Token>) -> (Vec<ASTNode>, Vec<String>) {
    let mut args: Vec<ASTNode> = vec![];
...
fn parse_gate_body(tokens: Vec<Token>) -> (Vec<Vec<f32>>, usize){
...
fn parse_gate(tokens: Vec<Token>) -> (ASTNode, usize) {
    let name = tokens[1].value.clone();
...
fn parse_gate_call(tokens: Vec<Token>, q: usize) -> (ASTNode, usize) {
    (ASTNode::GateCall {
        name: tokens[0].value.clone(),
        args: parse_arguments(tokens[2..].to_vec()).0,
...
fn parse_range(tokens: Vec<Token>) -> (ASTNode, usize) {
    (ASTNode::Range {
...
fn parse_for(tokens: Vec<Token>, q: usize) -> (ASTNode, usize) {
    let is_range = tokens[..tokens.iter().position(|x| x.token == 5).unwrap()].to_vec().contains(&Token {
        token: 15,
        value: String::from(".."),
    });
    ...
    if !is_range {
        collection = parse_rval(tokens[i + 3..tokens.iter().position(|x| x.token == 4).unwrap()].to_vec()).0;
    } else {
        collection = parse_range(tokens[tokens.iter().position(|x| is_num(x.value.clone())).unwrap()..=tokens.iter().position(|x| is_num(x.value.clone())).unwrap() + 2].to_vec().clone()).0;
    }
...
fn parse_assignment(tokens: Vec<Token>, q: usize) -> (ASTNode, usize) {
    if tokens[1].token == 12 {
        (ASTNode::Assignment {
            name: tokens[0].value.clone(),
            value: Box::new(parse_rval(tokens[2..tokens.iter().position(|x| x.token == 10).unwrap()].to_vec()).0),
            arr_index: None,
        }, parse_rval(tokens[2..].to_vec()).1 + q + 3)
    } else {
        (ASTNode::Assignment {
            name: tokens[0].value.clone(),
            arr_index: Some(Box::new(ASTNode::ArrayIndex(tokens[2].value.parse().expect("this should be a number...")))),
            value: Box::new(parse_rval(tokens[5..tokens.iter().position(|x| x.token == 10).unwrap()].to_vec()).0),
        }, parse_rval(tokens[5..].to_vec()).1 + q)
    }
}

fn parse_51_2(tokens: Vec<Token>, q: usize) -> (ASTNode, usize) {
    if tokens[..tokens.iter().position(|x| x.token == 10).unwrap()].to_vec().contains(&Token { token: 12, value: String::from("=") }) {
...
fn get_types(tokens: Vec<Token>) -> (Vec<ASTNode>, usize) {
...
fn parse_struct_def(tokens: Vec<Token>, q: usize) -> (ASTNode, usize) {
    (ASTNode::Struct {
        name: tokens[1].value.clone(),
